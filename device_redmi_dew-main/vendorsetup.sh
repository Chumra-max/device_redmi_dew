...
if [ "$1" = "$FDEVICE" -o "$FOX_BUILD_DEVICE" = "$FDEVICE" ]; then
    ...
    # --- vendor_boot-as-recovery (device confirmed to flash recoveries via vendor_boot,
    # not boot). OrangeFox devs mark this EXPERIMENTAL: bootloop/brick risk is real.
    # Recovery gets flashed via fastbootd:
    #   fastboot flash vendor_boot:recovery vendor_ramdisk_recovery.cpio
    # NOT by flashing the zip installer directly, unless you're holding stock
    # boot.img + vendor_boot.img as a recovery path. ---
    ...
    # FOX_VERSION is obsolete as of R11.3+ (hard build error). The OrangeFox
    # version number is now set internally by the source tree. The only
    # customisable version field left is the maintainer patch number below —
    # bump it by 1 each time you release a new build of the same base version.
